...
##### Modified for adversarial training
class XLNetModel(object):
  """A wrapper of the XLNet model used during both pretraining and finetuning."""

  def __init__(self, xlnet_config, run_config, input_ids, seg_ids, input_mask, adv_embedd=None,
    ...
    tfm_args = dict(
        ...
        adv_embedding_input=adv_embedd
    )
    ...
    with tf.compat.v1.variable_scope("model", reuse=tf.compat.v1.AUTO_REUSE):
        (self.output, self.new_mems, self.lookup_table, self.embed_output
  ...
  def get_embedding_output(self): ############################################
    """
    Returns:
      word embedding outputs. Used to adversarial training.
    """
