...
def positionwise_ffn(inp, d_model, d_inner, dropout, kernel_initializer,
  ...
  with tf.compat.v1.variable_scope(scope, reuse=reuse):
    ...
    output = tf.compat.v1.layers.dropout(output, dropout, training=is_training,
                               name='drop_2')
    output = tf.keras.layers.LayerNormalization(epsilon=1e-12, name='LayerNorm')(output + inp)
  return output
...
def post_attention(h, attn_vec, d_model, n_head, d_head, dropout, is_training,
  ...
  if residual:
    output = tf.keras.layers.LayerNormalization(epsilon=1e-12, name='LayerNorm')(attn_out + h)
  else:
    output = tf.keras.layers.LayerNormalization(epsilon=1e-12, name='LayerNorm')(attn_out)
...
def transformer_xl(inp_k, n_token, n_layer, d_model, n_head,
  ...
  with tf.compat.v1.variable_scope(scope):
    ...
    output_h = tf.compat.v1.layers.dropout(word_emb_k, dropout, training=is_training) ########## 필요
    
    if adv_embedding_input is not None:
        output_h = adv_embedding_input
    
    embed_output = output_h
    if inp_q is not None:
    ...
    return output, new_mems, lookup_table, embed_output ######################## xlnet.py 221
